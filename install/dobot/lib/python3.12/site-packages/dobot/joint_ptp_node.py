...
class JointPTPNode(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        while rclpy.ok():
            ...
            j1, j2, j3, j4 = dobot.get_joint_state()
            ...
            if prev_present[0] is not None:
                ...
                if all(diff < threshold_progress for diff in step_diffs):
                    self.get_logger().error("Action failed: Robot stopped moving before reaching goal. Possible obstacle collision.")
                    dobot.stop_current_action()
                    goal_handle.abort()
                    result.success = False
                    return result
